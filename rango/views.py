...
def index(request): 
    # Query the database for a list of ALL categories currently stored: (Category.objects).
    # Order the categories by the number of likes in descending order: (.order_by('-likes')).
    # Retrieve the top 5 only -- or all if less than 5: ([:5])
    category_list = Category.objects.order_by('-likes')[:5]
    
    #Getting the 5 most viewed pages
    ...
    #Handle cookies, Render the response, send back 
    visitor_cookie_handler(request)
...
def show_category(request, category_name_slug):
    ...
    try:
        ...
        # We then add the category object from the database to the context dictionary. 
...
def add_category(request): 
    ...
    if request.method == 'POST': 
        ...
        else: # The supplied form contained errors, just print them to the terminal. 
            print(form.errors)
...
def add_page(request, category_name_slug):
    try:
        ...
        category = None
    # You cannot add a page to a Category that does not exist... if category is None: return redirect('/rango/')
    if category is None:
        return redirect('/rango/')
    form = PageForm()
    if request.method == 'POST':
        form = PageForm(request.POST)
        ##Erorr here, says input http is not valid
        if form.is_valid():
            if category:
...
def register(request): 
    ...
    # Attempt to grab information from the raw form information. # Note that we make use of both UserForm and UserProfileForm. 
        ...
        profile_form = UserProfileForm(request.POST) # If the two forms are valid... 
        if user_form.is_valid() and profile_form.is_valid(): # Save the user's form data to the database. 
            user = user_form.save() # Now we hash the password with the set_password method. Once hashed, we can update the user object. 
            ...
            if 'picture' in request.FILES: 
                ...
                profile.save() # Update our variable to indicate that the template registration was successful. 
            registered = True 
        ...
        user_form = UserForm() 
        profile_form = UserProfileForm() # Render the template depending on the context. 
    return render(request, 'rango/register.html', context={'user_form': user_form, 'profile_form': profile_form, 'registered': registered})

def user_login(request): # If the request is a HTTP POST, try to pull out the relevant information. 
    if request.method == 'POST': # Gather the username and password provided by the user. # This information is obtained from the login form. 
        # We use request.POST.get('<variable>') as opposed to request.POST['<variable>'], because the request.POST.get('<variable>') returns None if the value does not exist, while request.POST['<variable>'] will raise a KeyError exception. 
        ...
        # If we have a User object, the details are correct. If None (Python's way of representing the absence of a value), no user with matching credentials was found. 
        if user: # Is the account active? It could have been disabled. 
            if user.is_active: # If the account is valid and active, we can log the user in. # We'll send the user back to the homepage. 
                login(request, user) 
...
def user_logout(request): 
    ...
    return redirect(reverse('rango:index')) # Take the user back to the homepage.
...
def visitor_cookie_handler(request): # Get the number of visits to the site. We use the COOKIES.get() function to obtain the visits cookie. If the cookie exists, the value returned is casted to an integer. 
